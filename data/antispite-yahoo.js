...
function wrapper() {
  (function(){
    ...
    var ajax = function(url,valueObj,method,cb_ok,cb_err) {
      ...
      // console.log("after");
      handlers[requestID] = cb_ok;
      requestID++;

    };

    self.port.on("gotMessage",function(data){
      handlers[data.requestID].apply(this,[data.params]);
    ...
    var analyticsTime = function(post){
      ...
      var beforeText = timestamp.textContent;
      return { 
    ...
    var analyticsPost = function(post,url){
        ...
        try{
          ...
          var username = userDom.textContent;
          var userkey = userDom.href.split("https://profile.yahoo.com/")[1];
          if(userkey == null){
            return null;
          }
          var content = post.querySelector(".ugccmt-commenttext").textContent;
    ...
    var handleYahooComment = function(){
      ...
      if(all_post_ids.length){
        doPost(SERVER+"comment/check",
          {
            posts:JSON.stringify(all_post_ids), //add-on only
            url:url
          },function(response){
                  ...
                  if(post.reply){
                    ...
                    if(post.reply.url){
                      ...
                      if(post.reply.url_title){
                        replyurl.textContent = "參考連結: " + post.reply.url_title;
                      }else{
                        replyurl.textContent = "參考連結: " + post.reply.url;
                      }
